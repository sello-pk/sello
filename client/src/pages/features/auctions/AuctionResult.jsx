...
const Button = ({ children, variant = "default", className = "", ...props }) => {
  ...
  return <button className={`inline-flex items-center justify-center font-medium px-6 py-3 text-sm transition-all duration-300 rounded-lg focus:outline-none ${v[variant]} ${className}`} {...props}>{children}</button>;
};
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-500 mb-6">This car doesn't have auction results yet.</p>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden mb-8">
            <div className="grid md:grid-cols-2">
              ...
              <div className="p-6 flex flex-col justify-center">
                <h2 className="text-xl font-bold text-slate-900 mb-2">{car.year} {car.make} {car.model}</h2>
                <div className="space-y-2 text-sm text-slate-600 mb-4">
                  <p>{car.mileage?.toLocaleString()} km • {car.fuelType} • {car.transmission}</p>
                </div>
                <Badge className="bg-emerald-100 text-emerald-700 w-fit"><CheckCircle className="w-3 h-3 mr-1" />Won</Badge>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-8">
            <h3 className="font-semibold text-lg text-slate-900 mb-4 flex items-center gap-2"><Wallet className="w-5 h-5 text-[#FFA602]" />Payment Breakdown</h3>
            <div className="space-y-3">
              ...
              <div className="flex justify-between py-3 border-b border-slate-100">
                ...
                <span className="font-semibold text-emerald-600">- {formatPrice(tokenDeduction)}</span>
              </div>
              <div className="flex justify-between py-3 bg-[#FFA602]/5 -mx-6 px-6 rounded-xl">
                <span className="font-semibold text-slate-900 text-lg">Amount Due</span>
                <span className="font-bold text-[#FFA602] text-xl">{formatPrice(amountDue)}</span>
              </div>
            </div>
          </div>
          ...
          {/* Pickup */}
          <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-8">
            <h3 className="font-semibold text-lg text-slate-900 mb-4 flex items-center gap-2"><MapPin className="w-5 h-5 text-[#FFA602]" />Vehicle Pickup</h3>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Link to="/auctions/transactions" className="flex-1">
              <Button className="w-full"><Wallet className="w-5 h-5 mr-2" />View My Transactions</Button>
            </Link>
            <Link to="/auctions" className="flex-1">
              <Button variant="outline" className="w-full">Back to Auctions<ArrowRight className="w-5 h-5 ml-2" /></Button>
            </Link>
          </div>
        </motion.div>
      </div>
