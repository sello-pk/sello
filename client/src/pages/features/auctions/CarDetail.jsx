...
const Button = ({ children, variant = "default", size = "default", className = "", ...props }) => {
  const base = "inline-flex items-center justify-center font-medium transition-all duration-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50";
  const v = {
...
const CountdownTimer = ({ targetDate, size = "default" }) => {
  ...
  return (
    <div className={`flex items-center justify-center gap-2 ${size === "large" ? "text-3xl" : "text-2xl"} font-bold text-white`}>
      {[{ v: time.d, l: "Days" }, { v: time.h, l: "Hours" }, { v: time.m, l: "Mins" }].map((t, i) => (
        <React.Fragment key={t.l}>
          {i > 0 && <span>:</span>}
          <div className="text-center"><span>{pad(t.v)}</span><span className="block text-xs font-normal text-slate-400">{t.l}</span></div>
        </React.Fragment>
      ))}
    </div>
  );
...
export default function CarDetail() {
  ...
  const { data: detail, refetch } = useGetAuctionCarDetailQuery(auctionCarId, { skip: !auctionCarId, pollingInterval: 10000 });
  ...
  const images = Array.isArray(car.images) ? car.images : [car.images].filter(Boolean);
  ...
  const specs = [
    ...
    { label: "Mileage", value: `${car.mileage?.toLocaleString() || 0} km`, icon: Gauge },
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <Link to="/auctions/live" className="inline-flex items-center text-slate-600 hover:text-slate-900">
            <ChevronLeft className="w-5 h-5 mr-1" />Back to Auction
          </Link>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-white rounded-2xl overflow-hidden border border-slate-200">
              <div className="relative aspect-[16/10] bg-slate-100">
                {images[currentImageIndex] && (
                  <img src={images[currentImageIndex]} alt={`${car.make} ${car.model}`} className="w-full h-full object-cover" />
                )}
                {images.length > 1 && (
                  <>
                    <button onClick={() => setCurrentImageIndex((p) => (p > 0 ? p - 1 : images.length - 1))} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg hover:bg-white"><ChevronLeft className="w-6 h-6" /></button>
                    <button onClick={() => setCurrentImageIndex((p) => (p < images.length - 1 ? p + 1 : 0))} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg hover:bg-white"><ChevronRight className="w-6 h-6" /></button>
                  </>
                )}
                <button onClick={() => setShowGallery(true)} className="absolute right-4 bottom-4 px-4 py-2 bg-white/90 rounded-lg flex items-center gap-2 text-sm font-medium hover:bg-white">
                  <ZoomIn className="w-4 h-4" />View All Photos
                </button>
                <div className="absolute left-4 bottom-4">
                  <Badge className="bg-black/70 text-white border-0 backdrop-blur"><MapPin className="w-3 h-3 mr-1" />{auction.location || "Okara Auction Yard"}</Badge>
                </div>
              </div>
              {images.length > 1 && (
                <div className="p-4 flex gap-2 overflow-x-auto">
                  {images.map((img, i) => (
                    <button key={i} onClick={() => setCurrentImageIndex(i)} className={`w-20 h-14 rounded-lg overflow-hidden flex-shrink-0 border-2 transition-all ${currentImageIndex === i ? "border-[#FFA602]" : "border-transparent"}`}>
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200">
              <h1 className="text-2xl md:text-3xl font-bold text-slate-900 mb-2">{car.year} {car.make} {car.model}</h1>
              ...
              {detail.reservePrice && (
                <div className={`rounded-xl p-4 ${currentHigh >= detail.reservePrice ? "bg-emerald-50 border border-emerald-200" : "bg-amber-50 border border-amber-200"}`}>
                  <div className="flex items-center gap-2">
                    {currentHigh >= detail.reservePrice ? (
                      <><CheckCircle className="w-5 h-5 text-emerald-600" /><span className="font-medium text-emerald-700">Reserve price met</span></>
                    ) : (
                      <><AlertTriangle className="w-5 h-5 text-amber-600" /><span className="font-medium text-amber-700">Reserve not yet met</span></>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {specs.map((spec, i) => (
                  <div key={i} className="bg-slate-50 rounded-xl p-4">
                    ...
                    <p className="font-semibold text-slate-900 capitalize">{spec.value || "N/A"}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {Object.keys(inspection).some((k) => k !== "notes" && inspection[k]) && (
              <div className="bg-white rounded-2xl border border-slate-200 p-6">
                <h3 className="font-semibold text-lg mb-4">Inspection Report</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  {Object.entries(inspection).filter(([k]) => k !== "notes" && k !== "_id").map(([key, val]) => val && (
                    <div key={key} className="text-center">
                      <div className={`rounded-xl p-4 border ${inspColors[val] || ""}`}>
                        {val === "pass" ? <CheckCircle className="w-8 h-8 mx-auto mb-2" /> : <AlertTriangle className="w-8 h-8 mx-auto mb-2" />}
                        <p className="text-xs uppercase tracking-wide mb-1">{key}</p>
                        <p className="font-semibold capitalize">{val.replace("_", " ")}</p>
                      </div>
                    </div>
                  ))}
                </div>
                {inspection.notes && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
                    <Info className="w-4 h-4 inline mr-1" />{inspection.notes}
                  </div>
                )}
              </div>
            )}
            ...
            {isLoggedIn && (
              <div className="bg-white rounded-2xl p-6 border border-slate-200">
                <div className="flex flex-col sm:flex-row gap-3">
                  <Button variant="outline" className={`flex-1 ${isFollowing ? "bg-red-50 border-red-300 text-red-600" : "border-[#FFA602] text-[#FFA602] hover:bg-[#FFA602]/10"}`} onClick={toggleFollow}>
                    {isFollowing ? <><HeartOff className="w-5 h-5 mr-2" />Unfollow</> : <><Heart className="w-5 h-5 mr-2" />Follow Car</>}
                  </Button>
                </div>
              </div>
            )}
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div className="p-6 border-b border-slate-200">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Badge className="bg-emerald-100 text-emerald-700">{auction.status === "live" ? "Live" : auction.status}</Badge>
                  </div>
                  <p className="text-3xl font-bold text-slate-900 mb-2">{formatPrice(currentHigh)}</p>
                  {detail.reservePrice && <p className="text-sm text-slate-500">Reserve: {formatPrice(detail.reservePrice)}</p>}
                </div>
                <div className="p-6">
                  <p className="text-sm font-medium text-slate-700 mb-3">Recent Bids</p>
                  <div className="space-y-3 max-h-60 overflow-y-auto mb-4">
                    {bids.length === 0 && <p className="text-sm text-slate-400 text-center py-4">No bids yet. Be the first!</p>}
                    {bids.slice(0, 8).map((bid, i) => (
                      <div key={bid._id || i} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                        <div>
                          <p className="font-medium text-slate-900">{bid.bidderName || bid.bidder?.name || "Anonymous"}</p>
                          <p className="text-xs text-slate-500">{new Date(bid.createdAt).toLocaleTimeString()}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-[#FFA602]">{formatPrice(bid.amount)}</p>
                          <Badge variant="default" className="text-xs bg-slate-100">
                            {bid.bidType === "offline" ? "Floor" : bid.isProxy ? "Auto" : "Online"}
                          </Badge>
                        </div>
                      </div>
                    ))}
                      ...
                      {!isLoggedIn ? (
                        ...
                        <div className="space-y-2">
                          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-700 text-center">
                            Auction access approval is required before bidding (status: {bidderStatus.replaceAll("_", " ")}).
                          </div>
                        ...
                        <>
                          ...
                          <p className="text-xs text-slate-500 text-center">Minimum increment: PKR 50,000</p>
                        </>
                      )}
                    </div>
                  )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-2xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowProxyBidForm(false)} className="p-2 hover:bg-slate-100 rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <p className="text-slate-600 mb-4">Set maximum amount you're willing to bid. We'll bid automatically up to this amount.</p>
            <div className="space-y-4">
              <div>
                <label className="text-sm text-slate-600 mb-1 block">Maximum Bid Amount</label>
                <input type="number" className="w-full px-4 py-2 border border-slate-200 rounded-lg" value={proxyMax} onChange={(e) => setProxyMax(Number(e.target.value))} min={currentHigh + 50000} step="50000" />
              </div>
              <Button className="w-full" onClick={handleProxyBid}>Set Proxy Bid</Button>
              <p className="text-xs text-slate-500 text-center">You'll be notified if you're outbid</p>
            </div>
          </motion.div>
        </div>
      )}
      ...
      <AnimatePresence>
        {showGallery && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black z-50 flex items-center justify-center">
            <button onClick={() => setShowGallery(false)} className="absolute top-4 right-4 w-12 h-12 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20"><X className="w-6 h-6 text-white" /></button>
            {images[currentImageIndex] && <img src={images[currentImageIndex]} alt="" className="max-w-full max-h-full object-contain" />}
            {images.length > 1 && (
              <>
                <button onClick={() => setCurrentImageIndex((p) => (p > 0 ? p - 1 : images.length - 1))} className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20"><ChevronLeft className="w-6 h-6 text-white" /></button>
                <button onClick={() => setCurrentImageIndex((p) => (p < images.length - 1 ? p + 1 : 0))} className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20"><ChevronRight className="w-6 h-6 text-white" /></button>
              </>
            )}
          </motion.div>
        )}
