...
/**
 * Generate OTP (alias for generateVerificationCode for backward compatibility)
 */
export const generateOtp = generateVerificationCode;
...
export const sendVerificationCode = async (phoneNumber, code) => {
  try {
    ...
    if (!/^\+?\d{9,15}$/.test(normalizedPhone)) {
      throw new Error(
        "Invalid phone number format. Must be 9-15 digits with optional country code.",
      );
    }
    ...
    if (twilioAccountSid && twilioAuthToken && twilioPhoneNumber) {
      ...
      try {
        ...
        if (twilioError.code === 21211) {
          Logger.error("Invalid phone number format for Twilio", {
            phoneNumber: normalizedPhone.substring(0, 4) + "****",
          });
          throw new Error("Invalid phone number format");
        } else if (twilioError.code === 21608) {
          Logger.error("Twilio phone number not verified (trial account)", {
            phoneNumber: normalizedPhone.substring(0, 4) + "****",
          });
          throw new Error(
            "Phone number not verified. Please verify your phone number in Twilio console.",
          );
          ...
          throw new Error(
            "SMS service authentication failed. Please check Twilio credentials.",
          );
        } else {
      ...
      Logger.info(
        `SMS verification code for ${normalizedPhone.substring(0, 4) + "****"}: ${code}`,
        {
          message:
            "SMS service not configured. Code logged for development only.",
          phoneNumber: normalizedPhone.substring(0, 4) + "****",
          code,
        },
      );
      ...
      if (process.env.NODE_ENV === "production") {
        throw new Error(
          "SMS service not configured. Please contact administrator.",
        );
      }
