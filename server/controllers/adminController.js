...
const AdminService = {
  getStats: async () => {
    ...
    // Basic counts
    const [users, cars, dealers, sold, blogs, requests, revenueData] =
      await Promise.all([
        User.countDocuments(),
        Car.countDocuments({ isApproved: true, status: "active" }),
        User.countDocuments({ role: "dealer", status: "active" }),
    ...
    const changes = {
      ...
      dealers: await getGrowth(User, { role: "dealer", status: "active" }),
    ...
    const salesTrends = timeline.map((m) => {
      const found = salesTrendsRaw.find(
        (r) => r._id.month === m.monthNum && r._id.year === m.year,
      );
    ...
    const userGrowth = timeline.map((m) => {
      const monthly = userGrowthRaw.filter(
        (r) => r._id.month === m.monthNum && r._id.year === m.year,
      );
      const newUsers = monthly
        .filter((r) => r._id.role === "individual")
        .reduce((a, b) => a + b.count, 0);
      const newDealers = monthly
        .filter((r) => r._id.role === "dealer")
        .reduce((a, b) => a + b.count, 0);
      return {
        month: m.month,
        newUsers,
  ...
  reviewCar: async (carId, isApproved, reason, adminId) => {
    ...
    try {
      await sendEmail(
        car.postedBy.email,
        `Listing ${statusText}`,
        template(car.postedBy.name, car.title, reason),
      );
    } catch (e) {
      Logger.warn("Moderation email failed", e);
    }
    ...
    const dashboardData = {
      metrics: [
        ...
        },
        {
          ...
          value: stats.cars,
          change: stats.changes.cars,
          icon: "listings",
        },
        {
          title: "Active Blogs",
          value: stats.blogs,
          change: stats.changes.blogs,
          icon: "blogs",
        },
        {
...
export const approveCar = async (req, res) => {
  try {
    ...
    const car = await AdminService.reviewCar(
      req.params.carId,
      isApproved,
      rejectionReason,
      req.user._id,
...
export const reviewAuctionAccessRequest = async (req, res) => {
  try {
    ...
    if (!allowedTypes.includes(type) || !allowedActions.includes(action)) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid review payload" });
    }
    ...
    const updateCapability = (key) => {
      ...
      user.auctionCapabilities[key].rejectionReason =
        action === "approve" ? "" : rejectionReason;
    };

    if (type === "auctionBidder" || type === "both")
      updateCapability("auctionBidder");
    if (type === "auctionDealer" || type === "dealer" || type === "both")
      updateCapability("auctionDealer");
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to review request" });
  }
};

export const getListingHistory = async (req, res) => {
  try {
    const cars = await Car.find({ postedBy: req.params.userId }).sort({
      createdAt: -1,
    });
