...
export const subscribeNewsletter = async (req, res) => {
  try {
    ...
    try {
      await sendEmail(
        normalizedEmail,
        "Welcome up Sello Newsletter!",
        "<h1>Welcome!</h1><p>You have subscribed successfully.</p>",
      );
    } catch (e) {
      Logger.warn("Newsletter email failed", e);
    }
...
export const getAllSubscribers = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      data: { subscribers, total, pages: Math.ceil(total / limit) },
    });
  } catch (err) {
...
export const getBlogBySlug = async (req, res) => {
  try {
    ...
    // For admin access, return any blog
    const isAdmin = req.user && req.user.role === "admin";
